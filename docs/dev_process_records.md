...
```
基础 RAG 链路实现后验证效果->rag_full_test_results.md，成功率75%
失败问题根因分析：
1. 行级 row-chunk 切割导致表格信息碎片化
  Q4 最典型：TRAN_SPEED 表有"频率单位编码"和
  "乘数编码"两个子表，检索分别命中了不同
  chunk，但 LLM
  上下文里缺少另一半，无法拼出完整答案。

2. 精确字段检索靠语义向量天然劣势
  Q2 查"CACHE_CTRL [33] bit0"，向量相似度对
  寄存器索引号"33"不敏感。BGE-M3 嵌入"bit0
  的含义"时，会优先匹配语义相似的描述文字，
  而不是精确的寄存器偏移量。
